...
const collector = message.channel.createMessageCollector(filter, { time: 999999999 });
    	var username = args[1];
    	if (username){
        roblox.getIdFromUsername(username).then(id => {
          ...
          message.channel.send(new Discord.RichEmbed().setTitle("Please put the following token in your profiles description").setDescription(`**${tokenID}**`).setFooter("When you have done that, say done").setColor("#ff4757")).then(() => {
            message.channel.awaitMessages(filter, { maxMatches: 1, time: 999999999, errors: ['time']})
            .then(collected => {
    ...
    console.log(message.author.id)
     ...
     console.log("successful")
         ...
         message.member.addRole(message.guild.roles.find(role => role.name === "Verified"));
                         ...
                         message.channel.send(new Discord.RichEmbed().setTitle("Error").setDescription(`**Cannot find code on Status**`).setFooter("Verification").setColor("#ff4757"))
         }
  })
